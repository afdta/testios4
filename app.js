...
function table(container, table_body_height){
	var table_columns = [];
	...
	var table_height = arguments.length > 1 ? (parseInt(table_body_height) + "px") : null;
	...
	var tfoot = wrap.append("div").style("margin-top","0.5em");
	...
	var col_index = -1;
	T.column = function(key, alias, ascending, formatter, set_default_sort, hide_on_mobile){
		var col = {};
		...
		col.mobile = !hide_on_mobile ? true : false;
		...
		else if(!!set_default_sort){sortcol = col;}
	...
	//ensure cell widths fill 100%
	var get_widths = function(mobile){
		try{
			var new_widths = cell_widths.filter(function(d,i,a){
				return !mobile || !!mobile && table_columns[i].mobile;
			});
			var n = new_widths.length;
			var w;
			if(n > 0){
				var sum = d3.sum(new_widths);
				var d = 100 - sum;
				var add = (d/n);
				w = new_widths.map(function(d,i){
					return Math.floor(d+add);
				});
			}
			else{
				throw "E";
	...
	//to do - re-work the error condition -- this won't mutate data...
	function sort_row_data(row_data, index, index_tb){

		var row_data_input = row_data.slice(0);
		...
		try{
			...
			row_data = [[]];
			var row_data_return = row_data_input;
		}

		return row_data_return;
	}

	var current_break = null;
	T.build = function(){
		try{
			var columns = table_columns.filter(function(d,i,a){
				return !current_break || (!!current_break && d.mobile);
			});
		}
		catch(e){
			var columns = table_columns;
		}
		...
		//sort data -- determine the indexes of the sortcol (and reset sort col if necessary)
		try{
			var keys = columns.map(function(d,i){return d.key});
			var index = keys.indexOf(sortcol.key);
			if(index < 0){throw "badIndex"}
		}
		catch(e){
			//reset sortcol
			var index = 0;
			sortcol = columns[index];
		}

		try{
			var index_tb = keys.indexOf(last_sortcol.key);
			if(index_tb < 0){throw "badIndex"}
		}
		catch(e){
			//reset sortcol tiebreak
			var index_tb = 0; 
			last_sortcol = columns[index_tb];
		}

		sort_row_data(rows, index, index_tb);

		//unit test: columns get shown/hidden as necessary
		//console.log("Current sort (" + index + ") "+ JSON.stringify(sortcol))
		//console.log("Last sort (" + index_tb + ") "+ JSON.stringify(last_sortcol))
		...
		thd.html(function(d,i){
				return !!current_break ? d.nameshort : d.name;
			})
		...
		thd.style("text-align", function(d,i){
		  ...
		  .classed("sort-asc", function(d,i){
		  	return d.key==sortcol.key && sortcol.asc;
		  })
		...
		var new_cell_widths = get_widths(!!current_break);

		if(new_cell_widths.length >= columns.length){
			thd.style("width", function(d,i){return new_cell_widths[i] + "%"});
			tbd.style("width", function(d,i){return new_cell_widths[i] + "%"});
		}
		else{
			thd.style("width", "auto");
			tbd.style("width", "auto");			
		}
		...
		//record that data has been initialized in table
		data_is_new = false;
	...
	var breakfn = function(){
		...
		current_break = b;
		if(!data_is_new){
			T.build(); //rebuild if the current data has been initialized once
		}
	};
	T.breaks = function(b){
		var firstRun = breaks.length==0;

		breaks = breaks.concat(b);
		...
		if(firstRun){
			...
			window.addEventListener("resize", function(){
				...
				resizeTimer = setTimeout(breakfn, 150);
			});
		}
	};
	...
	T.resize = function(){
		setTimeout(function(){
			try{				
				...
				tbd.style("font-size", fs);
				thd.style("font-size", fs);

				//to do: set table height for mobile screen
...
function metro_map(container, url, state, indicators){
	...
	var init = false;
	var current_indicator = indicators[0];
	function drawIndicator(ind, format$$1){
		var periods = {"five":"2010–2015", "ten":"2005–2015", "one":"2014–2015"};
		...
		map.format(!!format$$1 ? format$$1 : "pct1")
		   .smaller_better(ind.key==="relpov")
		   .bubble(ind.key,ind.key);
	}
...
//v1.0 developed for congressional district poverty, gig economy
...
function place_tip(tip_node, container_node, xbuffer, ybuffer, fbr){
	
	//default to showing tooltip on right, don't flip orientations unless forced
	var tipRight = true;
	var pad = arguments.length > 2 ? xbuffer : 0;
	fbr = arguments.length > 4 ? fbr : 85;
		...
		try{
			...
			if(tipRight){
				if((mouseX + tipdim.width + pad) > contdim.width){
					//tipRight = false;
					var newX = contdim.box.right - tipdim.width - pad - 1;
				}
				else{
					var newX = mouseX + pad;
				}
...
function definitions(container, showonly){
	var colors = {};
...
function header(){
	...
	O.build = function(container_element, callback){
		...
		if(arguments.length > 0){
			...
			var content = inner.append("div").classed("content-box full-bleed",true).style("padding","0em 1em");
...
function ranks_and_distributions(container, url, state, category){
  ...
  chart.textwrap = chart.wrap_outer.append("div").style("padding","1em 1em");
  ...
  function build(){
    ...
    //add in labels
    var labels_update = chart.label_container.selectAll("div").data(indicators);
    ...
    (function(){
      try{
        ...
        txt.enter().append("text").merge(txt)
            ...
            .text(function(d,i){
              return d.val === null ? "" : format.pct1(d.val)
            });

        var boxtext = chart.textwrap.selectAll("p.data-text").data([lookup.rank].concat(lookup.values));
        boxtext.enter().append("p").classed("data-text",true).merge(boxtext).html(function(d,i){
          var possessive = format.possessive(metname(cbsa).nameshort);
          if(i==0){
            return possessive + " overall rank on " + category + ": " + (d==null ? "N/A" : format.rank(d));
          }
          else{
            var prefix = i==1 ? "<br /><em>" + possessive + " percent change in</em>: <br/>" : "";
            return prefix + d.variable + ": " + (d.val==null ? "N/A" : "<strong>"+format.pct1(d.val)) + "</strong> (" + (d.rank==null ? "N/A" : format.rank(d.rank)) + " of 100)"
          }
        })
        .classed("big-strong", function(d,i){return i==0})
        .style("line-height",function(d,i){return i==0 ? "1.5em" : "2em"})
        .style("margin","0em");

        chart.textwrap.selectAll("p.note").remove();
        if(category=="inclusion"){
          chart.textwrap.append("p").text("Note: Declines in relative poverty are a good outcome.").style("margin","1.5em 1em 0em 0em").classed("note",true);
        }
...
function shift_shares(container, url, state){
	...
	.html('Did aggregate job growth in the ' + state.metro_lookup(state.metro).nameshort + 
		' metro area outperform or underperform the nation<span class="current-period"></span>?');
	//hold chart in wrap
	...
	chart.wrap_outer.append("p")
		.html('Which industries outperformed or underperformed on job growth<span class="current-period"></span>?')
		.classed("chart-title",true)
		.style("margin","5em 0em 0em 0em")
		;

	chart.wrap_outer.append("p")
		.text("Showing industries with at least 0.5% of all jobs in 2015; see above for definitions of expected growth, actual growth, and the competitive shift")
		;
		
	chart.tablewrap = chart.wrap_outer.append("div")
		.classed("content-box",true)
		;

	chart.table = table(chart.tablewrap.node(), 500);

	var periods = chart.wrap_outer.selectAll("span.current-period");

	//chart.table.column("naics", "NAICS", true, null, null, true);
	chart.table.column("ind", "Industry", true);
	chart.table.column("expected", "Expected growth", false, format.fn0("pct1"));
	chart.table.column("actual", "Actual growth", false, format.fn0("pct1"));
	chart.table.column("cs", "Competitive shift", false, format.fn0("pct1"), null, true);
	chart.table.column("share", "Share of jobs, 2015", false, format.fn0("sh1"), true);

	chart.table.widths([37,15,15,15,18]);
	...
	function build(){
		...
		var naics2name = this.get_data("ilookup");
		var ps = {"five":"2010–2015", "ten":"2005–2015", "one":"2014–2015"};
		periods.text(" from " + ps[period]);

		var deets = json.detail[metro].map(function(d,i){
			return {
					ind: naics2name(d.naics), 
					expected:(d["exp"+period]), 
					actual:(d["exp"+period] + d["cs"+period]),
					cs:d["cs"+period],
					share:d.share15}
		});

		chart.table.data(deets).build();
		...
		var shifts = [
						...
						{val:(data.ns+data.im), pct:(data.ns+data.im)/data.e0, label:"pro"},
						{val:(data.ns+data.im+data.cs), pct:(data.ns+data.im+data.cs)/data.e0, label:"act"},
						{val:data.cs, pct:data.cs/data.e0, label:"cs"}
					];
		...
		var bars = barsE.merge(barsU)
			...
			//.style("background-color","#eeeeee")
			.style("border-bottom",function(d,i){return i < 2 ? "2px solid #ffffff" : "none"});

		var barsV = bars.select("div.value-bar")
			.style("left",function(d,i){
				var x = scaleX(d.val/e0);
				var w = Math.abs(scaleX(d.val/e0) - zero);
				if(x >= zero){
					var l = zero+"%";
				}
				else{
		...
		var svglabels = bars.select("svg");

		var labels = svglabels.selectAll("text").data(function(d,i){
									   		return [d,d];
									   	});

		labels.enter().append("text").merge(labels)
			.text(function(d,i){return format.pct1(d.pct)})
			.attr("y","80%")
			.attr("x",90)
			.attr("text-anchor","end")
			.attr("stroke", function(d,i){
				return i == 0 ? (d.val < 0 ? "#dc2a2a" : "#0d73d6") : "none";
			})
			.attr("stroke-width", function(d,i){return i==0 ? 4 : 0})
			.attr("fill","#ffffff");
		...
		var text = bars.select("p").html(function(d,i){
			...
			if(i==0){
				return "<strong>Expected job growth</strong>: The job change that would have occurred if every industry in the metro area matched the national growth rate in that industry."
			}
			else if(i==1){
				return "<strong>Actual job growth</strong>: The actual job change in the metro area."
			}
			else if(i==2){
				return "<strong>Metro area competitive shift</strong>: The difference between the actual job growth and the expected job growth. It indicates whether the metro area overperformed or underperformed given its industrial structure."
			}
	...
	function init(){
		var look = this.get_data().lookup;
		var lookup = {};
		var i=-1;
		while(++i < look.length){
			lookup[look[i].naics] = look[i].industryname.replace(/^\s*/,"");
		}
		var lookupfn = function(code){
		...
		this.set_data(lookupfn, "ilookup");
	...
	function updateMetro(){
		...
		' metro area outperform or underperform the nation?');
...
function trend_lines(container, url, state){
  ...
  chart.wrap_outer = d3.select(container).classed("card-box makesans",true);
  chart.textwrap = chart.wrap_outer.append("div").style("padding","1em 0em 2em 2em").classed("right-col",true);
  chart.wrap = chart.wrap_outer.append("div").classed("content-box", true).append("div"); //.style("margin","1em 10%");
...
chart.groups.select("text.plot-title")
...
function section_header(g_scope){
	...
	var build = function(container, title, subtitle_fn){
		...
		var wrap = d3.select(container)
			...
			.style("padding","4em 1em 0.5em 1em")
		...
		if(subtitle_fn !== null){
			...
			var subfn = function(){
				var html = arglen < 3 ? "In <strong>" + 
									lookup(g_scope.metro).name + 
									"</strong> / " + g_scope.periods[g_scope.period] :
	...
	var buildMain = function(container){
		...
		var wrap = d3.select(container)
			...
			.style("border-bottom", "1px solid rgb(220, 42, 42)")
			.style("border-bottom", "0px solid #333333")
			.append("div")
			.classed("content-box",true)
...
function inclusion_detail(container, url, state){

	//keep track of dom
	var dom = {};
	dom.wrap = d3.select(container)
		.style("width","100%")
		.style("margin","4em auto 0em auto")
		.style("min-height","500px")
		.classed("m-i makesans",true)
		;
	dom.titlebox = dom.wrap.append("div").style("margin","0em 0em 0em 4%");
	dom.titlebox.append("p").classed("big-strong",true).text("How have changes in inclusion varied by race and education?");

	var guide = dom.titlebox.append("div");
	guide.append("p").style("display","inline-block").text("The box plots below illustrate the following metro area percentiles: ")
					 .style("margin","0em")
					 .style("line-height","1em")
					 .style("vertical-align","top")
					 ;
	var guidesvg = guide.append("div")
						.style("width","300px")
						.style("display","inline-block")
						.style("height","2.5em")
						.style("vertical-align","top")	
	...
	guidesvg.selectAll("text").data(["5th","25th","50th","75th","95th"]).enter().append("text").attr("x", function(d,i){
		var is = [40, 100, 140, 190, 270];
		return is[i];
	}).attr("y", 27)
	...
	guidesvg.append("path").attr("d","M40,7 l60,0 m90,0 l80,0").attr("stroke","#333333").attr("stroke-dasharray","4,2");
	guidesvg.append("path").attr("d","M100,0 l90,0 M100,14 l90,0").attr("stroke","#333333");
	guidesvg.append("path").attr("d","M40,0 l0,14 m60,-14 l0,14 m40,-14 l0,14 m50,-14 l0,14 m80,-14 l0,14").attr("stroke","#333333");
	guidesvg.selectAll("path").attr("stroke","#333333").style("shape-rendering","crispEdges");


	//guide to box and whiskers plots


	var metro_lookup = metro_select().lookup;
	...
	function build(){
	    ...
	    var groups = this.get_data("groups");

	    var mains = this.get_data("mains");

	    mains.each(function(d,i){
	    	var wrap = d3.select(this);
	    	var label = d.key == "epop" ? d.label : d.label;
	    	wrap.select("p.plot-title")
	    		.text(label + " by race and education")
	    		.style("font-weight","bold")
	    		.style("text-align","left");

	    	wrap.select("p.selected-metro-area").text(metro_lookup(cbsa).nameshort);

	    	//console.log(d);

	    	drawIncDetail(d.val[period], d.extent[period], cbsa, groups, wrap);
	    });
	...
	function init(){
		...
		var rollextent = d3.nest()
				...
				.rollup(function(d){
					//var extent = d3.extent(d, function(d){return d.Value});
					var sd = d3.deviation(d, function(d){return d.Value});
					var mean = d3.mean(d, function(d){return d.Value});
					var outliers = [mean-(5*sd), mean+(5*sd)];

					var rm5sd = [];
					var i = -1;
					while(++i < d.length){
						if(d[i].Value >= outliers[0] && d[i].Value <= outliers[1]){
							rm5sd.push(d[i]);
						}
					}

					var numremoved = (d.length - rm5sd.length);
					
					return {extent: d3.extent(rm5sd, function(d){return d.Value}), outliers:numremoved};
				});

		var rolled = rollup.object(data.obs);
		var extents = rollextent.object(data.obs);

		var mains = dom.wrap.append("div").selectAll("div.plot-wrap").data([
			{key:"epop", label:"Percent change in the employment rate", val:rolled.epop, extent:extents.epop},
			{key:"medearn", label:"Percent change in median wage", val:rolled.medearn, extent:extents.medearn},
			{key:"relpov", label:"Percent change in relative poverty", val:rolled.relpov, extent:extents.relpov}
		]);
		mains.exit().remove();

		var mains_enter = mains.enter().append("div").classed("plot-wrap",true);
		var mains2 = mains_enter.merge(mains).style("margin",function(d,i){return i==0 ? "3em 0em 0em 0em" : "4em 0em 0em 0em"});

		//add in legend
		mains2.select("div.legend-box").remove();

		var legend = {};
		legend.wrap = mains2.append("div").classed("c-fix legend-box",true).style("border-bottom","1px solid #aaaaaa").style("padding","0em 2% 0.5em 2%").style("margin","0em 2%");
		legend.wrap.append("p").classed("plot-title",true).style("float","left").style("margin","0em 2em 0em 0em");
		...
		legend.selected.append("div").style("width","16px").style("height","1.25em").style("float","left")
					  ...
					  .append("path").attr("d",function(d,i){
					  	return "M" + (6.5+(10*i)) + ",7 l5,9 l-10,0 z"
					  }).attr("fill",function(d,i){
					  	return "#dc2a2a";
					  })
					  ;
		legend.selectedGeo = legend.selected.append("p").classed("selected-metro-area",true).style("float","left").text("Selected metro area").style("margin","0em");

		legend.average = legend.wrap.append("div").style("float","left").classed("c-fix",true).style("opacity","1");
		legend.average.append("div").style("width","16px").style("height","1.25em").style("float","left")
					  .append("svg").attr("width","100%").attr("height","100%")
					  .selectAll("circle").data([1]).enter()
					  .append("circle").attr("cx",6.5).attr("cy",11).attr("r",3.5).attr("fill","#666666")
					  ;

		legend.average.append("p").style("float","left").text("U.S. average").style("margin","0em");


		this.set_data(mains2, "mains");
		this.set_data(data.groups, "groups");
	    this.set_data(data, "raw");
		this.set_data(rolled);
	...
	function drawIncDetail(data, extent_object, cbsa, groups, wrap){
		var extent = extent_object.extent;
		var outliers = extent_object.outliers;

		var races = groups.race.slice(1);
		var edus = groups.edu.slice(2);

		var RD = races.map(function(d,i){
			return {key:d, vals:data.Total[d]};
		});

		var ED = edus.map(function(d,i){
			return {key:d, vals:data[d].Total};
		});

		var lineheight = 52; //height of each data line
		var grouppad = 60; //spacing between race and edu groupings
		var svgpad = 30; //extra pad at top and bottom of svg
		var plotheight = (RD.length+ED.length)*lineheight;

		var groupScale = d3.scaleBand().domain(races.concat(edus)).range([0,plotheight]).round(true).paddingOuter(0.75).paddingInner(0.5);
		var val_scale = d3.scaleLinear().domain(extent).range([5,95]);
		var valScale = function(v, addl){
			if(v < extent[0]){
				var x = val_scale(extent[0]);
				return {x: x+"%", X:x, outlier:true, position:-1}
			}
			else if (v > extent[1]){
				var x = val_scale(extent[1]);
				return {x: x+"%", X:x, outlier:true, position:1}
			}
			else{
				var x = val_scale(v);
				return {x: x+"%", X:x, outlier:false, position:0}
			}
		};

		var svg = wrap.selectAll("svg.outer-svg").data([[{label:"By race", vals:RD}, {label:"By educational attainment", vals:ED}]]);
			svg.exit().remove();
		var svg_enter = svg.enter().append("svg").classed("outer-svg", true);
			svg_enter.append("g").classed("axis-layer",true);
			svg_enter.append("g").classed("plot-layer",true);
			

		var svg_merged = svg_enter.merge(svg).attr("height",(plotheight + grouppad + (2*svgpad)) + "px").attr("width","100%"); 

		var signote = wrap.selectAll("p.stat-sig-note").data(["*Indicates a statistically significant change."]);
		signote.enter().append("p").classed("stat-sig-note",true).merge(signote).style("text-align","right").raise().text(function(d,i){return d}).style("margin","0em 2%");
		
		var main_groups = svg_merged
							 .select("g.plot-layer")
							 .selectAll("g.main-groups")
							 .data(function(d,i){
							 	return d;
							 });

			 
			main_groups.exit().remove();
		var main_groups_enter = main_groups.enter().append("g").classed("main-groups",true);
			main_groups_enter.append("g").classed("h_gridline_layer",true);
			main_groups_enter.append("g").classed("place_layer",true);
			main_groups_enter.append("g").classed("place_block_layer",true).style("pointer-events","none");
			main_groups_enter.append("g").classed("v_gridline_layer",true);
			main_groups_enter.append("g").classed("box_whiskers_layer",true);
			main_groups_enter.append("g").classed("label_layer",true).append("line").style("pointer-events","none");
			main_groups_enter.append("g").classed("avg_layer",true).attr("transform","translate(1.5,0)").style("pointer-events","none");
			main_groups_enter.append("g").classed("anno_layer",true).attr("transform","translate(-405,0)").style("pointer-events","none");

		var groups = main_groups_enter.merge(main_groups)
				.attr("transform",function(d,i){return "translate(0," + ((i*grouppad)+svgpad) + ")"});
		...
		var D = dots.enter().append("rect").merge(dots)
			...
			.attr("y",0)
			.attr("fill", function(d,i){
				return d.CBSA=="99999" ? "#dc2a2a" : "#0d73d6";
			})
			.attr("fill-opacity",function(d,i){
				return d.CBSA=="99999" ? 0 : 0.3;
			})
			.attr("stroke-opacity","0.5")
			.attr("stroke-width","0")
			.attr("stroke", function(d,i){
		...
		var blanks = groups.select("g.place_block_layer")
							.selectAll("rect").data(function(d,i){
								return d.vals.map(function(d){return d.key});
							});
		...
		//plot markings
		//horizontal axes
		var hlines = groups.select("g.h_gridline_layer").selectAll("line").data(function(d,i){
			return d.vals;
		});
		hlines.exit().remove();
		...
		.attr("y1", function(d){return groupScale(d.key)+5})
		.attr("y2", function(d){return groupScale(d.key)+5})
		.attr("stroke","#dddddd")
		.attr("stroke-width","1")
		...
		//x-axis
		var tickLayer = svg_merged.select("g.axis-layer");
		var tickValues = val_scale.ticks(6).map(function(d){
				return [{v:d, x:val_scale(d)+"%", y2:plotheight+grouppad, i:0}, {v:d, x:val_scale(d)+"%", y2:plotheight+grouppad, i:1}]
			});

		var vlu = tickLayer.selectAll("g.vline").data(tickValues);
			vlu.exit().remove();
		var vle = vlu.enter().append("g").classed("vline",true);
			vle.append("line").classed("tick",true).attr("stroke","#aaaaaa").attr("stroke-width","1").style("shape-rendering","crispEdges");
			vle.append("text").attr("text-anchor","middle").style("font-size","0.8em").attr("fill","#666666");
		var vlines = vle.merge(vlu);

		vlines.select("line.tick")
			.attr("x1", function(d,i){return d[0].x})
			.attr("x2", function(d,i){return d[0].x})
			.attr("y1", function(d,i){return d[0].y2+2})
			.attr("y2", function(d,i){return d[0].y2+9})
			;

		vlines.select("text")
			.text(function(d,i){
				return format.pct0(d[0].v);
			})
			.attr("x", function(d,i){return d[0].x})
			.attr("y", function(d,i){return d[0].y2+21})
			;		


		//vertical grid lines
		var plot_g_height = [races.length*lineheight, edus.length*lineheight];
		var v_lines = groups.select("g.v_gridline_layer").selectAll("line").data(function(d,i){
			return tickValues.map(function(d){
				return d[i];
			})
		}); 
			v_lines.exit().remove();
			v_lines.enter().append("line").merge(v_lines).attr("stroke","#dddddd")
											.attr("stroke-width","1")
											//.attr("stroke-dasharray","2,3")
											.style("shape-rendering","crispEdges")
											...
											.attr("y1", function(d,i){
												return d.i==0 ? svgpad : plot_g_height[0];
											})
											.attr("y2", function(d,i){
												return d.i==0 ? plot_g_height[0] : plot_g_height[0] + plot_g_height[1] - svgpad;
											})
											;
		...
		.attr("x", pctBounds2[0])
		.attr("y", function(d){return groupScale(d.key)-3})
		.attr("text-anchor","start")
		...
		//label by-categories - race or edu
		var catLabels = [{y:groupScale(races[0])-svgpad, label:"Race/ethnicity"},
						 {y:groupScale(edus[0])-svgpad, label:"Educational attainment"}];
		...
		var bw = groups.select("g.box_whiskers_layer").selectAll("g.box-and-whiskers").data(function(d,i){
			return d.vals;
		});

		var box_groups = bw.enter().append("g").classed("box-and-whiskers",true).merge(bw).attr("transform", function(d,i){
			return "translate(0," + groupScale(d.key) + ")";
		}); 

		box_groups.each(function(d,i){
			var vals = d.vals.filter(function(d,i){
				return d.CBSA != "99999";
			}).map(function(d,i){
				return d.Value;
			}).sort(function(a,b){
			...
			var p25 = d3.quantile(vals, 0.25);
			var p50 = d3.median(vals);
			var p75 = d3.quantile(vals, 0.75);
			var p5 = d3.quantile(vals, 0.05);
			var p95 = d3.quantile(vals, 0.95);

			var x25 = val_scale(p25) + "%";
			var x50 = val_scale(p50) + "%";
			var x75 = val_scale(p75) + "%";
			var x5 = val_scale(p5) + "%";
			var x95 = val_scale(p95) + "%";

			var center = 5;
			var center_offset = 5;
			//horizontal segments
			//5th to 25th
			lines.push({x1:x5, x2:x25, y1:center, y2:center, linetype:"dotted", note:null});
			//inner quartiles (25th to 75th)
			lines.push({x1:x25, x2:x75, y1:center-center_offset, y2:center-center_offset, linetype:"solid", note:null});
			lines.push({x1:x25, x2:x75, y1:center+center_offset, y2:center+center_offset, linetype:"solid", note:null});
			//75th to 95th
			lines.push({x1:x75, x2:x95, y1:center, y2:center, linetype:"dotted", note:null});

			//vertical segments
			lines.push({x1:x5, x2:x5, y1:center-center_offset, y2:center+center_offset, linetype:"solid", note:"lowerBound"});
			lines.push({x1:x25, x2:x25, y1:center-center_offset, y2:center+center_offset, linetype:"solid", note:"p25Bound"});
			lines.push({x1:x50, x2:x50, y1:center-center_offset, y2:center+center_offset, linetype:"solid", note:"p50Bound"});
			lines.push({x1:x75, x2:x75, y1:center-center_offset, y2:center+center_offset, linetype:"solid", note:"p75Bound"});
			lines.push({x1:x95, x2:x95, y1:center-center_offset, y2:center+center_offset, linetype:"solid", note:"upperBound"});

			var bg = d3.select(this);

			var B = bg.selectAll("rect").data([1]);
			B.exit().remove();
			B.enter().append("rect").merge(B)
				.attr("x", x25)
				.attr("y", center-center_offset)
				.attr("width", (val_scale(p75)-val_scale(p25))+"%")
				.attr("height",center_offset*2)
				.attr("fill", "#fafafa")
				.attr("stroke", "none");

			var L = bg.selectAll("line").data(lines);
			L.exit().remove();
			L.enter().append("line").merge(L)
					...
					.attr("stroke-dasharray", function(d,i){
						return d.linetype == "dotted" ? "3,3" : null;
					})
		...
		var avg_dots = groups.select("g.avg_layer")
			.selectAll("circle")
			.data(function(d,i){
				var prop = d.label == "By race" ? "Race" : "Education";
				return d.vals.map(function(d,i){
					var us = d.vals.filter(function(d,i){
						return d.CBSA == "99999";
					})[0];					
					return {val:us.Value, group:us[prop]};
				})
			});
		...
		var avgs = avg_dots.enter().append("circle")
						.merge(avg_dots)
						.attr("cy",function(d,i){
							return groupScale(d.group)+19;
						})
						.attr("cx","50%")
						.attr("r",3.5)
						.attr("fill", function(d,i){return "#666666"})
						.style("opacity",1)
						.attr("cx",function(d,i){
							return valScale(d.val).x;
						});
			...
			var svgs_u = groups.select("g.anno_layer").selectAll("svg").data(function(d,i){
				...
				var m = d.vals.map(function(d,i){
					...
					var j = -1;
					while(++j < d.vals.length){
						if(d.vals[j].CBSA == "99999"){
							us = d.vals[j];
						}
						else if(d.vals[j].CBSA == cbsa){
							metro = d.vals[j];
						}
					}

					r.value = metro !== null ? metro.Value : null;
					r.se = metro !== null ? metro.SE : null;

					var sig = (r.value !== null && r.se !==null) ? ((r.value/r.se) < -1.645 || (r.value/r.se) > 1.645) : false; 

					var x = metro !== null ? valScale(metro.Value) : {x:"-10%", X:50, position:0};
					
					var text = r.value !==null ? format.pct1(r.value) : "N/A";
					r.text = x.outlier ? text + " (outlier)" : text;
					r.sig = sig ? "*" : "";

					r.above_average = r.value === null || r.value >= us.Value;
					
					r.x = x.x;
					r.outlier = x.position;

					if(x.X > 85 || x.X < 15){r.above_average = !r.above_average;}
			...
			svgs.select("path")
				...
				.attr("d", function(d,i){
					if(d.outlier == 0){
						//only path that needs to point to value precisely
						return "M406.5,37 l6.5,12 l-13,0 z";
					}
					else if(d.outlier == 1){
						//point right to outlier
						//test case: deltona 5yr
						return "M405,38 l12,6.5 l-12,6.5 z";
					}
					else{
						//point left to outlier
						//test case: north port, 1yr
						return "M410,37 l-12,6.5 l12,6.5 z";
					}
				});
			...
			svgs.select("text.value")
				.attr("text-anchor",function(d,i){
					...
					return d.above_average ? 414 : 398;
				})
				.text(function(d,i){
					return d.text + d.sig;
				})
...
//dir.local("./").add("json17", "data/2017/json")
...
scope.metro = "47900";
scope.period = "five";
scope.periods = {one:"2014–2015", five:"2010–2015", ten:"2005–2015"};
...
function main(){
	...
	var iselect_wrap = header_content.append("div")
			...
			.style("margin","0em")
			.classed("c-fix makesans",true);
	var menu_tiles_title = iselect_wrap.append("p")
		.style("font-size","0.8em")
		.style("margin","0em")
		.style("line-height","1em")
		.style("display","inline-block")
	...
	var menu_tiles = iselect_wrap
			.append("div")
			.classed("c-fix",true)
			.selectAll("div.ranking-tile").data([
				{id:"growth", label:"Growth", anchor:"#growth-section"},
				{id:"prosperity", label:"Prosperity", anchor:"#prosperity-section"},
				{id: "inclusion", label:"Inclusion", anchor:"#inclusion-section"}
				]).enter().append("div")
			.style("float","left")
			.style("width","100px")
			.style("height","80px")
	...
	var menu_tile_links = menu_tiles.append("a")
								.attr("href", function(d,i){return d.anchor})
								.style("border","none")
	...
	var menu_tile_labels = menu_tile_links.append("p").text(function(d,i){return d.label})
			  .style("font-size","0.8em")
			  .style("margin","0.5em")
			  .style("line-height","1em")
			  .style("text-align","center");

	var menu_tile_ranks = menu_tile_links.append("p").text(function(d,i){return d.label})
	...
	var tselect_wrap = selects.append("div")
			.style("float","right")
			.style("margin","0em 0em 0em 2em");

	var mselect_wrap = selects.append("div")
		.style("float","right");
	...
	//indicate which section is being viewed
	var inview = {
					growth:{pos:null, id:"growth"}, 
					prosperity:{pos:null, id:"prosperity"}, 
					inclusion:{pos:null, id:"inclusion"}
				};
	var current_view = null;

	var pickone = function(height){
		try{
			var coverage = 0;
			var view = null;

			for(var p in inview){
				var et = inview[p].pos.top < 0 ? 0 : (inview[p].pos.top > height ? height : inview[p].pos.top);
				var eb = inview[p].pos.bottom < 0 ? 0 : (inview[p].pos.bottom > height ? height : inview[p].pos.bottom);
				if((eb - et) > coverage && (eb-et)/height >= 0.5){
					coverage = eb-et;
					view = inview[p].id;
				}
			}
		...
		if(view !== current_view){
			menu_tiles.style("border", function(d,i){
				return d.id==view ? "1px solid #dc2a2a" : "1px solid #aaaaaa";
			});

			menu_tile_labels.style("color", function(d,i){
				return d.id==view ? "#dc2a2a" : null;
			});

			menu_tile_ranks.style("color", function(d,i){
				return d.id==view ? "#dc2a2a" : null;
			});
	...
	mhead.view(function(position){
		if(position==1){
			menu_tiles.style("height","52px").style("margin-top","5px");
			menu_tiles_title.style("display","none");
			menu_tile_ranks.style("font-size","1em");
		}
		else{
			menu_tiles.style("height","80px").style("margin-top","0px");
			menu_tiles_title.style("display","inline-block");
			menu_tile_ranks.style("font-size","1.5em");
		}
	});

	var growth_section = document.getElementById("growth-section");
	var prosperity_section = document.getElementById("prosperity-section");
	var inclusion_section = document.getElementById("inclusion-section");

	var onscroll = function(){
		var window_height = Math.max(document.documentElement.clientHeight, (window.innerHeight || 0));
		inview.growth.pos = growth_section.getBoundingClientRect();
		inview.prosperity.pos = prosperity_section.getBoundingClientRect();
		inview.inclusion.pos = inclusion_section.getBoundingClientRect();
		pickone(window_height);
	};
	...
	var onresize = function(){
		var box = mhead.content.node().getBoundingClientRect();
		var w = box.right - box.left;
		if(w < 600){
			iselect_wrap.style("display","none");
			selects.style("float","left");
			mselect_wrap.style("font-size","0.8em").style("max-width","180px");
			tselect_wrap.style("font-size","0.8em").style("max-width","90px");
		}
		else if(w < 780){
			menu_tiles.style("width","65px");
			selects.style("float","right");
			mselect_wrap.style("font-size","0.8em").style("max-width","180px");
			tselect_wrap.style("font-size","0.8em").style("max-width","90px");
		}
		else if(w < 950){
			menu_tiles.style("width","75px");
			selects.style("float","right");
			iselect_wrap.style("display","block").style("font-size","0.8em");
			mselect_wrap.style("font-size","0.8em").style("max-width",null);
			tselect_wrap.style("font-size","0.8em").style("max-width",null);
		}
		else{
			menu_tiles.style("width","100px");
			selects.style("float","right");
			iselect_wrap.style("display","block").style("font-size","1em");	
			mselect_wrap.style("font-size","1em");
			tselect_wrap.style("font-size","1em");		
		}
	};
	window.addEventListener("resize", onresize);
	onresize();

	var overall_ranks = null; 
	var populate_overall_ranks = function(){
		var fn = function(dat){
			var d = dat[scope.period][scope.metro][0];
			menu_tile_ranks.text(function(indicator){
				var r = d["rnk_"+indicator.id];
				return format.rank(r);
			});
		};
		if(overall_ranks == null){
			d3.json(dir.url("json17", "overall_ranks.json"), function(error, data){
				overall_ranks = data;
				fn(overall_ranks);
			});
		}
	...
	sectionHeader.factory(document.getElementById("methods-section-header"), "Methodology notes", null);
	
	sectionHeader.factorySpecial(document.getElementById("data-section-header"));

	//populate overall ranks
	...
	cards.growth3 = metro_map(growth_card_3, 
								...
								scope, 
								[{key:"emp", title:"Jobs"},
								 {key:"output", title:"Gross metropolitan product (GMP)"},
								 {key:"emp5u", title:"Jobs at young firms"}
								]);
	...
	cards.prosperity3 = metro_map(prosperity_card_3, 
								...
								scope, 
								[{key:"gdpjob", title:"Productivity"},
								 {key:"gdpcap", title:"Standard of living"},
	...
	cards.inclusion3 = metro_map(inclusion_card_3, 
								...
								scope, 
								[{key:"medearn", title:"Median wage"},
								 {key:"relpov", title:"Relative poverty"},
								 {key:"epop", title:"Employment rate"}
								]);

	var inclusion_card_4 = document.getElementById("inclusion-card4");
	cards.inclusion4 = inclusion_detail(inclusion_card_4,
										 dir.url("json17", "inclusion_detail.json"),
										 scope);
	cards.inclusion4.activate();
